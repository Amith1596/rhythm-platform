...
# Register new productivity models
@admin.register(ProductivityMetrics)
class ProductivityMetricsAdmin(admin.ModelAdmin):
    list_display = ('contributor', 'focus_hours', 'meeting_load', 'flow_index', 'burnout_risk')
    list_filter = ('burnout_risk', 'work_style')
    search_fields = ('contributor__username',)

@admin.register(TeamCollaboration)
class TeamCollaborationAdmin(admin.ModelAdmin):
    list_display = ('contributor_a', 'contributor_b', 'interaction_type', 'interaction_count', 'last_interaction')
    list_filter = ('interaction_type',)
    search_fields = ('contributor_a__username', 'contributor_b__username')

@admin.register(MockCalendarData)
class MockCalendarDataAdmin(admin.ModelAdmin):
    list_display = ('contributor', 'date', 'event_type', 'duration_hours', 'title')
    list_filter = ('event_type', 'date')
    search_fields = ('contributor__username', 'title')